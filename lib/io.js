...
var Sockets = function(server, wind, ee){
    ...
    if(process.env.REDISTOGO_URL){
        ...
        var pub = redis(process.env.REDIS_PORT, process.env.REDIS_HOST, { auth_pass: process.env.REDIS_PASS });
        var sub = redis(process.env.REDIS_PORT, process.env.REDIS_HOST, { detect_buffers: true, auth_pass: process.env.REDIS_PASS });
        io.adapter(adapter({ pubClient: pub, subClient: sub }));
    } else {
        var redis = require('socket.io-redis');
        io.adapter({ host: 'localhost', port: 6379 });
    }
